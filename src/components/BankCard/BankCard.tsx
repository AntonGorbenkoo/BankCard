import { Button } from '../../assets/components/Button/Button';
import classes from './BankCard.module.sass';
import { useRef, useState } from 'react';

export const BankCard = () => {
  const [valueNumber, setValueNumber] = useState<string>('****************');
  const [valueName, setValueName] = useState<string>('Ivan Ivanov');
  const [valueDate, setValueDate] = useState<string>('****');
  const [valueCvv, setValueCvv] = useState<string>('420');
  const [addForm, setAddForm] = useState<boolean>(false);
  const [frontSide, setFrontSide] = useState<boolean>(false);
  const [validNum, setValidNum] = useState<boolean>(false);
  const [validName, setValidName] = useState<boolean>(false);
  const [validDate, setValidDate] = useState<boolean>(false);
  const [validCvv, setValidCvv] = useState<boolean>(false);
  const inpNumber = useRef<HTMLInputElement>(null);
  const inpName = useRef<HTMLInputElement>(null);
  const inpDate = useRef<HTMLInputElement>(null);
  const inpCvv = useRef<HTMLInputElement>(null);

  //Value Number*****************************
  const onInputNumber = (e: any) => {
    const value = e.target.value;
    let replacedValue = value.replace(/\*/g, '');
    if (/^\d*$/.test(replacedValue)) {
      if (replacedValue.length <= 16) {
        replacedValue = replacedValue.padEnd(16, '*');
      } else {
        replacedValue = replacedValue.slice(0, 16);
      }
    } else {
      replacedValue = valueNumber;
    }

    setValueNumber(replacedValue);

    if (/^\d{16}$/.test(replacedValue)) {
      inpNumber.current?.classList.remove(`${classes.invalid}`);
      inpNumber.current?.classList.add(`${classes.valid}`);
      setValidNum(true);
    } else {
      setValidNum(false);
      inpNumber.current?.classList.remove(`${classes.valid}`);
      inpNumber.current?.classList.add(`${classes.invalid}`);
    }
  };

  //value Name ***************************
  const onInputName = (e: any) => {
    const inputName = e.target.value;
    const words = inputName.trim().split(/\s+/);
    if (words.length === 2) {
      const [firstName, lastName] = words;
      setValueName(`${firstName} ${lastName}`);
    } else if (words.length > 2) {
      const [firstName, ...rest] = words;
      setValueName(`${firstName} ${rest.join(' ')}`);
    } else {
      setValueName(inputName);
    }

    if (/^[a-zA-Z\s]+$/.test(inputName)) {
      inpName.current?.classList.add(`${classes.valid}`);
      inpName.current?.classList.remove(`${classes.invalid}`);
      setValidName(true);
    } else {
      inpName.current?.classList.add(`${classes.invalid}`);
      inpName.current?.classList.remove(`${classes.valid}`);
      setValidName(false);
    }
  };

  //value Date ***************************
  const onInputDate = (e: any) => {
    const value = e.target.value;
    let replacedValue = value.replace(/\*/g, '');
    if (/^\d*$/.test(replacedValue)) {
      if (replacedValue.length <= 4) {
        replacedValue = replacedValue.padEnd(4, '*');
      } else {
        replacedValue = replacedValue.slice(0, 4);
      }
    } else {
      replacedValue = valueDate;
    }
    setValueDate(replacedValue);

    if (/^\d{4}$/.test(replacedValue)) {
      setValidDate(true);
      inpDate.current?.classList.remove(classes.invalid);
      inpDate.current?.classList.add(classes.valid);
    } else {
      setValidDate(false);
      inpDate.current?.classList.remove(classes.valid);
      inpDate.current?.classList.add(classes.invalid);
    }
  };

  //Value CVV *********************************

  const onInputCvv = (e: any) => {
    const regExp = /^[0-9]{3}$/;
    if (regExp.test(e.target.value) && e.target.value.length === 3) {
      setValueCvv(e.target.value);
      inpCvv.current?.classList.add(classes.valid);
      inpCvv.current?.classList.remove(classes.invalid);
      setValidCvv(true);
    } else {
      setValidCvv(false);
      inpCvv.current?.classList.remove(classes.valid);
      inpCvv.current?.classList.add(classes.invalid);
    }
  };

  const onFocusInputCvv = () => {
    setFrontSide(true);
  };

  const onBlurInputCvv = () => {
    setFrontSide(false);
  };

  const handleClickBtnData = () => {
    setAddForm(true);
  };

  const handleClickClose = () => {
    setAddForm(false);
  };

  const handleClickReset = (e: any) => {
    e.preventDefault();
    if (
      inpNumber.current &&
      inpName.current &&
      inpDate.current &&
      inpCvv.current
    ) {
      inpNumber.current.value = '';
      setValueNumber('****************');
      setValidNum(false);
      inpNumber.current?.classList.remove(classes.invalid);
      inpNumber.current?.classList.remove(classes.valid);
      inpName.current.value = '';
      setValueName('Dima Ivanov');
      setValidNum(false);
      inpName.current?.classList.remove(classes.valid);
      inpName.current?.classList.remove(classes.invalid);
      inpDate.current.value = '';
      setValueDate('****');
      setValidDate(false);
      inpDate.current?.classList.remove(classes.valid);
      inpDate.current?.classList.remove(classes.invalid);
      inpCvv.current.value = '';
      setValueCvv('420');
      setValidCvv(false);
      inpCvv.current?.classList.remove(classes.valid);
      inpCvv.current?.classList.remove(classes.invalid);
    }
  };

  const handleSubmit = (e: any) => {
    e.preventDefault();
    if (validNum && validName && validDate && validCvv) {
      const newData = {
        CardNumber: inpNumber.current?.value,
        UserName: inpName.current?.value,
        CardDate: inpDate.current?.value,
        CardCvv: inpCvv.current?.value,
      };
      console.log(newData);
    }
  };

  return (
    <div className={classes.wrapper}>
      <div className={classes.cardsSides}>
        <div
          className={
            !frontSide
              ? classes.frontSide
              : `${classes.frontSide} ${classes.frontSideOne}`
          }
        >
          <div className={classes.cardPanel}>
            <div className={classes.cardNumber}>
              <div className={classes.one}>{valueNumber.slice(0, 4)}</div>
              <div className={classes.two}>{valueNumber.slice(4, 8)}</div>
              <div className={classes.three}>{valueNumber.slice(8, 12)}</div>
              <div className={classes.four}>{valueNumber.slice(12, 16)}</div>
            </div>

            <div className={classes.cardDate}>
              <div className="left">{valueDate.slice(0, 2)}</div>
              <div className="stick">/</div>
              <div className="right">{valueDate.slice(2, 4)}</div>
            </div>
            <div className={classes.info}>
              <div className={classes.owner}>{valueName.toUpperCase()}</div>
              <img
                className="card__logo"
                alt="#"
                src="data:image/png;base64,"
              />
            </div>
          </div>
        </div>
        <div
          className={
            !frontSide
              ? classes.backSide
              : `${classes.backSide} ${classes.backSideOne}`
          }
        >
          <div className={classes.cardBackPanel}>
            <div className={classes.blackLine}></div>
            <div className={classes.cvv}>
              <div className={classes.leftCvv}></div>
              <div className={classes.rightCvv}>
                <p>{valueCvv}</p>
              </div>
            </div>
            <div className={classes.visa}>
              <img
                className="card__logo"
                alt="#"
                src="data:image/png;base64,"
              />
            </div>
          </div>
        </div>
      </div>
      {!addForm ? (
        <Button className={classes.btnData} onClick={handleClickBtnData}>
          Enter data
        </Button>
      ) : (
        <div className={classes.newData}>
          <h3>Your card details</h3>
          <form onSubmit={handleSubmit}>
            <div className={classes.inputGroup}>
              <label>Card Number</label>
              <input
                ref={inpNumber}
                type="text"
                placeholder="55..., 55..., 22..., 44..."
                onChange={onInputNumber}
                maxLength={16}
                className={classes.inputNumber}
              />
              <label className={classes.labelName}>Name on card</label>
              <input
                ref={inpName}
                type="text"
                onChange={onInputName}
                maxLength={20}
                placeholder="Ivan Ivanov"
                className={classes.inputName}
              />
              <div className={classes.DateCvv}>
                <div className={classes.date}>
                  <label className={classes.labelDate}>Expiry date</label>
                  <input
                    ref={inpDate}
                    type="text"
                    onChange={onInputDate}
                    maxLength={4}
                    placeholder="Date"
                    className={classes.inputDate}
                  />
                </div>
                <div className={classes.cvv}>
                  <label>Security code</label>
                  <input
                    ref={inpCvv}
                    type="text"
                    onChange={onInputCvv}
                    onFocus={onFocusInputCvv}
                    onBlur={onBlurInputCvv}
                    maxLength={3}
                    placeholder="Your CVV"
                    className={classes.inputCvv}
                  />
                </div>
              </div>
            </div>
            <Button onClick={handleClickClose} className={classes.clsButton}>
              Close
            </Button>
            <Button onClick={handleClickReset} className={classes.resButton}>
              Reset form
            </Button>
            <Button className={classes.sendButton}>Pay</Button>
          </form>
        </div>
      )}
    </div>
  );
};
